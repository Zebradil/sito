...
| `tiers`                           | attrs     | `{}`                           | Named tiers, each `{ priority ? 1000; strategy ? null; upstreams.<name> = { priority ? 1000; url; public-keys ? []; }; }`, rendered into `settings.tier` sorted by `priority` then name. Named entries merge across modules, so a host can add an upstream to a tier defined elsewhere. Mutually exclusive with `settings.tier`. |
| `manageSubstituters`              | bool      | `true`                         | Point `nix.settings.substituters` at `http://<listen>` and set `nix.settings.trusted-public-keys` to every `public-keys` entry found across the tiers (flattened and deduped). Set false to curate both by hand. |
